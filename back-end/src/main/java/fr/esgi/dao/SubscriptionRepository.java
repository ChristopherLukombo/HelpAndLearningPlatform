...
public interface SubscriptionRepository extends JpaRepository<Subscription, Long> {
    ...
    @Query(value = "SELECT count(DISTINCT user_id) AS NUMBER_SUBSCRIBER FROM subscription WHERE trick_id = :trickId GROUP BY trick_id", nativeQuery = true)
    Integer findNumberSubcriber(@Param("trickId") Long trickId);
